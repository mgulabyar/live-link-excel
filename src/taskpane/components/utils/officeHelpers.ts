...
// Helper: Kisi bhi file path/URL se uski parent directory (folder path) nikalne ke liye
...
// UPDATED: Rename vs Copy detection with parent directory logic
export const getExistingLinkId = async (
  sheetName: string,
  rangeAddress: string,
    ...
    for (let item of xmlBlobs) {
      try {
        ...
        if (xmlText && xmlText.includes("LiveLink")) {
          ...
          // Normalization check for exact files
          const normalizedCurrent = normalizeUrl(currentFileId);
          const normalizedSaved = normalizeUrl(savedFileId);

          // Folder paths matching for renamed files
          const parentCurrent = getParentFolder(currentFileId);
          const parentSaved = getParentFolder(savedFileId);

          // RENAME VS COPY RULE:
          // Agar file name change hua hai lekin folder path same hai, to ye Rename hai aur hum link active rakhenge.
          // Agar folder path bilkul alag hai, to ye Copy hai aur naya link banega.
          ...
          if (!isSameFileOrRenamed) {
            continue; // Skip templates copied in other project folders
          }
...
// UPDATED & CLEANED: formatExcelRange completely empty check logic to prevent blue/gray styles
export const formatExcelRange = async (_sheetName: string, _rangeAddress: string) => {
  await Excel.run(async (context: any) => {
    // Intentionally left blank to fully prevent gray highlights and blue borders on Excel ranges
    await context.sync();
  });
};
