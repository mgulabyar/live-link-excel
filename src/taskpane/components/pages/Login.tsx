...
const Login: React.FC<LoginProps> = ({ onLoginSuccess, onNavigateToSignup }) => {
  ...
  useEffect(() => {
    if (localStorage.getItem("cb_justSignedUp") === "true") {
      setToast({
        open: true,
        message: "Registered! Please log in.",
        severity: "success",
      });
      localStorage.removeItem("cb_justSignedUp");
    }
  }, []);
  ...
  const handleAuth = () => {
    ...
    if (savedUser.email === email && savedUser.password === password) {
      ...
      setToast({ 
        open: true, 
        message: "Account not found. Redirecting...", 
        severity: "error" 
      });
      localStorage.setItem("cb_redirectToSignup", "true");
